(function() {
    ...
    DatePickerCtrl.prototype.configureNgModel = function(ngModelCtrl, mdInputContainer, inputDirective) {
        ...
        self.ngModelCtrl.$formatters.push(function(value) {
            //attempt to parse the date if it is a string
            //https://github.com/angular/material/issues/6253

            if (value && !(value instanceof Date)) {
                var parseValue = new Date(value);
                if (isNaN(parseValue)) {
                    throw Error('The ng-model for md-datepicker must be a Date instance. ' +
                        'Currently the model is a: ' + (typeof value));
                } else {
                    var utcValue = new Date(parseValue.getUTCFullYear(), parseValue.getUTCMonth(), parseValue.getUTCDate());
                    value = utcValue;
                }
            }
