...
angular.module('ngMaterial', ["ng","ngAnimate","ngAria","material.core","material.core.gestures","material.core.layout","material.core.theming.palette","material.core.theming","material.core.animate","material.components.autocomplete","material.components.backdrop","material.components.button","material.components.bottomSheet","material.components.card","material.components.content","material.components.checkbox","material.components.chips","material.components.datepicker","material.components.dialog","material.components.fabActions","material.components.divider","material.components.fabShared","material.components.fabSpeedDial","material.components.fabToolbar","material.components.fabTrigger","material.components.gridList","material.components.icon","material.components.input","material.components.list","material.components.menu","material.components.menuBar","material.components.progressCircular","material.components.progressLinear","material.components.showHide","material.components.radioButton","material.components.sidenav","material.components.slider","material.components.select","material.components.subheader","material.components.sticky","material.components.swipe","material.components.switch","material.components.tabs","material.components.toast","material.components.toolbar","material.components.tooltip","material.components.virtualRepeat","material.components.whiteframe"]);
})();
(function(){
"use strict";
...
function MdAutocompleteCtrl($scope, $element, $mdUtil, $mdConstant, $mdTheming, $window,
    ...
    var ctrl = this,
        ...
        allItemsParts = ($scope.allItemsExpr || '').split(/ in /i),
        allItemsExpr = allItemsParts.length > 1 ? allItemsParts[1] : itemParts[1],
        elements = null,
        cache = {},
        noBlur = false,
        ...
        hasFocus = false,
        lastCount = 0,
        showNotFound = $scope.showNotFound,
        alwaysShowNotFound = $scope.alwaysShowNotFound,
        fetchesInProgress = 0;
    ...
    ctrl.orderBy = $scope.orderBy;
    ctrl.orderByDescending = $scope.orderByDescending;
    ctrl.showArrow = $scope.showArrow;
    ctrl.alwaysShowNotFound = $scope.alwaysShowNotFound;
    ...
    ctrl.fetchAllResults = fetchAllResults;
    ctrl.mouseUp = onMouseup;
    ctrl.getCurrentDisplayValue = getCurrentDisplayValue;
    ctrl.getItemText = getItemText;
    ctrl.registerSelectedItemWatcher = registerSelectedItemWatcher;
    ctrl.unregisterSelectedItemWatcher = unregisterSelectedItemWatcher;
    ctrl.notFoundVisible = notFoundVisible;
    ctrl.loadingIsVisible = loadingIsVisible;
    ctrl.isShowingAll = false;
    ...
    function configureWatchers() {
        ...
        $scope.$on('hideAutoCompleteList', function () {
            $scope.$apply(function () {
                ctrl.hidden = true;
            });
        });
    ...
    function focus() {
        ...
        ctrl.hidden = shouldHide();
        if (!ctrl.hidden && !ctrl.isShowingAll) handleQuery();
    }
     ...
     * @returns {boolean}
     */
    function shouldHide(retrieveAll) {
        if (ctrl.loading && !hasMatches()) return true;                 // Hide while loading initial matches
        else if (!retrieveAll && hasSelection()) return true;           // Hide if there is already a selection
        else if (!retrieveAll && !hasFocus) return true;                // Hide if the input does not have focus
        else return !shouldShow(retrieveAll);                                      // Defer to standard show logic
    }
     ...
     * @returns {boolean}
     */
    function shouldShow(retrieveAll) {
        return retrieveAll || ((isMinLengthMet() && hasMatches()) || notFoundVisible());
    }
    ...
    /**
     * Handles "retrieve all"
     * @param event
     */
    function fetchAllResults() {
        positionDropdown();
        ctrl.isShowingAll = true;
        fetchResults('', true);
    }
     ...
     * @param searchText
     */
    function fetchResults(searchText, retrieveAll) {
        var items = $scope.$parent.$eval(retrieveAll === true ? allItemsExpr : itemExpr),
        ...
        function handleResults(matches) {
            cache[term] = matches;
            if (!retrieveAll && (searchText || '') !== ($scope.searchText || '')) return; //-- just cache the results if old request
            ...
            if (ctrl.loading) setLoading(false, retrieveAll);
    ...
    function notFoundVisible() {
        ...
        return alwaysShowNotFound || (showNotFound && ctrl.hasNotFound && !hasMatches() && (!ctrl.loading || isPromiseFetching()) && textLength >= getMinLength() && (hasFocus || noBlur) && !hasSelection());
    }
